...
import GameLogic from './Cells/MovingLogick'
...
export default function TetrisContainer(props) {
  ...
  function checkWay (horizontal, vertical, board, elemVariant, randomElem){
    ...
    for (let w = 0; w<=elemWidth; w++) {
      for (let h = 0; h<=elemHeight; h++) {
        if(TETRA_ELEMENTS[randomElem][elemVariant][h][w]===1){
          maxH = h
        }
      }
      positions.push(maxH)
  ...
  useEffect(() => {
    ...
    if(props.lose)clearInterval(timerRef.current)
